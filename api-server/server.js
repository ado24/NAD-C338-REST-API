...
// Batch endpoint for getting all state at once
app.get('/state', async (req, res) => {
    try {
        const [power, volume, source, mute, brightness, bass] = await Promise.all([
            nad.getPower(),
            nad.getVolume(),
            nad.getSource(),
            nad.getMute(),
        ...
        res.json({ power, volume, source, mute, brightness, bass });
    } catch (error) {
        res.status(500).send(error.message);
    }
});
...
app.patch('/power', async (req, res) => {
    try {
        ...
        // Return actual device state after PATCH
        const power = await nad.getPower();
        res.json({ power });
    } catch (error) {
        res.status(500).send(error.message);
